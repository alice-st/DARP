...
class TestDARP(unittest.TestCase):
    def test_DARP_success(self):
        with open(f'unitTests/test{self.case}_success.pickle', 'rb') as handle:
            groundTruthDARPsuccess = pickle.load(handle)
        assert_true(groundTruthDARPsuccess == self.multiRobot.DARP_success)  
    ...
    def test_DARP_assignment_matrix(self):   
        ...
        assert_true(np.allclose(self.multiRobot.darp_instance.A, groundTruthAssignmentMatrix, rtol=1e-02))
    ...
    def test_DARP_return_paths(self):
        ...
        assert_almost_equal(self.multiRobot.returnPaths, groundTruthReturnPaths)
